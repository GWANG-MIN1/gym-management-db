...
**이전 버전과 달라진 점** (기존 클라이언트·스크립트가 있다면 확인하세요)

| 변경 | 이유 |
|------|------|
| `POST /sessions` 가 `status` 를 받지 않음 | 예약은 항상 `SCHEDULED` 로 생성. 완료 처리는 잔여 횟수 차감을 동반하므로 `/complete` 로만 |
| `GET /members` · `/trainers` 가 기본 50건만 반환 | 전체 반환이 느려진 원인. 더 필요하면 `limit`(최대 200) / `offset` |
| 제약 위반이 500 → 409 / 404 / 422 | 전화번호 중복 등이 예외 처리 없이 500 으로 나가던 문제 |
| 오류 문구가 한국어로 바뀜 (`Member not found` → `회원을 찾을 수 없습니다.`) | 업무 오류를 읽고 조치할 수 있게 |
| `99:99` 같은 시각이 422 로 거부됨 | 이전 정규식이 통과시키던 값 |
...
| 파일 | 확인하는 것 |
|------|------------|
| `test_members.py` · `test_trainers_exercises.py` | 등록·조회, 전화번호/종목명 중복 409, 페이지네이션, 입력 검증 422 |
| `test_sessions.py` | 만료·잔여 횟수·중복 예약 차단, 완료 시 잔여 횟수 차감, 취소 후 재예약, 상태 전이 규칙 |
...
컨테이너가 시작하면서 운영 DB 에 DDL 이 실제로 실행됩니다.

1. **배포 전 RDS 스냅샷을 뜹니다.** downgrade 가 없으므로 되돌리는 수단은 스냅샷뿐입니다.
2. 배포 후 `/health` 가 200 이면 마이그레이션까지 끝난 것입니다.
3. 마이그레이션이 실패하면 컨테이너가 뜨지 않아 배포가 실패합니다.
   CD 가 기존 컨테이너를 먼저 내리는 구조라 그동안 API 가 중단되므로,
   `docker logs gym-api` 로 원인(대개 제약을 위반하는 기존 행)을 확인하고 데이터를 정리한 뒤
   다시 배포합니다.

### 업그레이드 경로를 검증하는 테스트

나머지 테스트는 모두 빈 DB 에서 시작하므로 "기존 DB 위에 새 버전을 배포하는" 경로를 확인하지
못합니다. `test_migration_from_legacy.py` 는 별도 DB 에 예전 스키마와 데이터를 만들어
다음을 확인합니다.

- 마이그레이션 **전에는** 등록이 422 로 실패한다는 것 (문제 재현)
- 마이그레이션 결과 스키마가 ORM 정의와 컬럼·제약·인덱스까지 일치하는 것
- 기존 행이 남고, SERIAL → IDENTITY 전환 후에도 ID 채번이 이어지는 것
- 등록·예약·취소 후 재예약, 새로 생긴 테이블 사용이 정상 동작하는 것
- 재배포로 두 번 실행돼도 안전한 것
